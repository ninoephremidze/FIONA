...
def _ensure_store_dir():
    """
    Return a Path to the GL2D cache directory, creating it if necessary.

    Returns None if FIONA_GL2D_DIR is unset or the directory cannot be created.
    """
...
def _gl2d_exists(n, u_max):
    """Return True if all required GL2D cache files for (n, u_max) exist on disk."""
...
def _compute_and_store_gl2d(
    ...
    Args:
        n:                      Number of GL points per dimension.
        u_max:                  Half-extent of the integration domain.
        nprocs:                 Number of worker processes to use.
        target_chunks_per_proc: Desired number of row-chunks per worker (controls
                                granularity of the parallel decomposition).
